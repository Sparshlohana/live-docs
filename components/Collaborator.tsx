...
const Collaborator = ({ roomId, creatorId, email, collaborator, user }: CollaboratorProps) => {
    ...
    const shareDocumentHandler = async (type: string) => {
        setLoading(true);
        await updateDocumentAccess({ roomId, email: email, userType: type as UserType, updatedBy: user });
        setLoading(false);
    }
        ...
        <li className='flex items-center justify-between gap-2 py-3'>
            ...
            {creatorId === collaborator.id ? (
                ...
                <div className="flex items-center">
                    ...
                    <Button type='button' onClick={() => removeDocumentHandler(collaborator.email)}>
                        Remove
                    </Button>
                </div>
            )}
